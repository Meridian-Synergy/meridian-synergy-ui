...
export type MsContactActionType = 'whatsapp' | 'email'

export interface MsContactAction {
  type:  MsContactActionType
  href:  string
  label: string
}
...
defineProps<{
  title:   string
  desc:    string
  actions: MsContactAction[]
}>()
</script>

<template>
  <section class="ms-contact-banner">
    <div class="ms-contact-banner__inner">
      <div class="ms-contact-banner__text">
        <h2 class="ms-contact-banner__title">{{ title }}</h2>
        <p class="ms-contact-banner__desc">{{ desc }}</p>
      </div>
      <div class="ms-contact-banner__actions">
        <a
          ...
          :class="['ms-contact-btn', `ms-contact-btn--${action.type}`]"
        >
          <!-- WhatsApp icon -->
...
.ms-contact-banner__text {
  flex: 1;
  min-width: 260px;
}

.ms-contact-banner__title {
...
.ms-contact-btn:hover { opacity: 0.88; transform: translateY(-1px); }

.ms-contact-btn--whatsapp {
  background: #25D366;
  color: #fff;
}

.ms-contact-btn--email {
  background: rgba(255, 255, 255, 0.1);
  color: var(--ms-color-white);
  border: 1px solid rgba(255, 255, 255, 0.2);
}
.ms-contact-btn--email:hover { background: rgba(255, 255, 255, 0.16); }
</style>
