...
<template>
  <a
    ...
    :class="['ms-contact-channel', `ms-contact-channel--${type}`]"
  >
    <div class="ms-contact-channel__icon">
      <!-- WhatsApp -->
      <svg v-if="type === 'whatsapp'" width="28" height="28" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    ...
    <div class="ms-contact-channel__body">
      <span class="ms-contact-channel__label">{{ label }}</span>
      <span class="ms-contact-channel__value">{{ value }}</span>
      <span class="ms-contact-channel__desc">{{ desc }}</span>
    </div>

    <span class="ms-contact-channel__cta">{{ cta }} →</span>
  </a>
</template>
...
.ms-contact-channel--whatsapp .ms-contact-channel__icon {
